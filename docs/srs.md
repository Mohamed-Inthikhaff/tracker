...
1.4 References
ù Feasibility Study: "From Spreadsheet to SaaS: Feasibility Study for an Expense Tracking Product" (companion document).
ù Technical Build Plan: "Technical Build Plan - Expense Tracker SaaS" (companion document).
ù Source artifact: "2026 Personal Expense Tracker" Google Sheet - the existing manual system this Product replaces; its Categories, Transactions, Debts, Budget, and Dashboard sheets are the primary source of current-state business logic referenced throughout Section 3.
ù IEEE 830-1998 / ISO/IEC/IEEE 29148:2018 - structural reference for this SRS.
...
ù Record income, expense, saving, and debt-related transactions manually or via receipt photo, SMS/notification capture, or CSV import.
ù Receive an AI-suggested category for each transaction and confirm or correct it with minimal friction.
ù Maintain a running ledger of informal debts owed to or by named individuals, with automatic reconciliation against repayment transactions.
ù Set monthly budgets per category and see live budget-vs-actual variance and a savings-rate calculation.
ù View a dashboard summarizing income, expense, saving, net balance, category breakdown, and multi-month trend for a selected month.
ù Share a Household's data with other invited members, with real-time updates across members.
ù Subscribe to a paid plan via a freemium billing model.
ù (Phase 3 / future) Link a bank account for automatic transaction import in supported markets.
...
ù Server-side: containerized NestJS services on AWS ECS Fargate; Postgres (Neon) as the primary data store; AWS S3 for object storage (receipts); Google Gemini for AI classification (and optionally receipt vision); Textract optional for structured OCR if not collapsed into Gemini multimodal.
ù Client-side (web): current versions of Chrome, Safari, Firefox, and Edge on desktop and mobile, at a minimum viewport width of 360px.
ù Client-side (mobile capture): installable PWA on Android and iOS; Android additionally supports optional notification-listener-based SMS capture (Section 3.8), which is not available on iOS due to platform restrictions.
ù Third-party dependencies: Auth0 (authentication), Stripe (billing), Google Gemini (AI categorization / receipt vision), AWS Textract (optional OCR), and, in Phase 3 only, Plaid or an equivalent aggregator (bank sync).
2.5 Design and Implementation Constraints
ù Bank-account aggregation (Plaid or equivalent) has no coverage in the initial target market (Sri Lanka / South Asia) as of this writing; consequently automated bank sync MUST NOT be a dependency of any Phase 1 or Phase 2 requirement. See FR-BANK-* in Section 3.12 for the explicitly scoped exception.
ù SMS/notification-based capture (Section 3.8) is an Android-only capability due to iOS platform restrictions on reading other apps' notifications; the system MUST degrade gracefully to manual/receipt-based capture on iOS.
ù The system MUST support multi-tenancy at the Household level from the first release; retrofitting shared access after single-user data models are in production is explicitly avoided as a design risk.
ù The backend MUST be implemented in NestJS/TypeORM and the web frontend in Next.js, consistent with the organization's existing engineering stack and operational tooling.
ù A native mobile application is not required for the version covered by this SRS; the PWA MUST fulfill all mobile capture requirements without it.
2.6 Assumptions and Dependencies
ù It is assumed that initial users are individuals or households already maintaining a manual spreadsheet or no formal tracking at all, and that CSV import from a spreadsheet is a primary onboarding path (see FR-IMP-*).
ù It is assumed that Google Gemini, Auth0, and Stripe remain available and within acceptable pricing for AI categorization, authentication, and billing respectively (Textract if used for OCR); a provider outage in any of these degrades but does not fully block core manual-entry functionality (see NFR-REL-002).
ù It is assumed that currency handling for the initial release is single-currency per Household (see Section 2.5 of the Technical Build Plan); multi-currency is a Phase 3 dependency, not a Phase 1 requirement.
...
4.1 User Interfaces
ù A responsive web application covering full dashboard, transaction management, budgets, debts, categories, and settings functionality, targeting desktop and tablet viewports.
ù A mobile-first installable PWA optimized for the quick-add capture flow (FR-TXN-002), receipt capture (Section 3.7), and, on Android, SMS/notification capture (Section 3.8).
ù All monetary values shall be displayed with the Household's base currency symbol and consistent decimal formatting throughout every screen.
ù Color-coded status indicators (budget health, debt status) shall be accompanied by a non-color cue (icon or label) to remain usable for color-vision-deficient users.
4.2 Hardware Interfaces
ù Camera access on the client device for receipt capture (Section 3.7); the system shall function fully without camera access via manual upload or manual entry as a fallback.
ù No other hardware interfaces are required.
...
Phase 1
Google Gemini
AI-assisted category suggestion (FR-CAT-003+) via `gemini-2.5-flash-lite`; optional multimodal receipt extraction + category mapping (FR-RCPT-002) ù may replace or reduce Textract.
Phase 1 (text); Phase 2 (vision)
AWS Textract
Optional structured OCR from receipt images if Gemini vision is not used for FR-RCPT-002.
Phase 2
AWS S3 / CloudFront
...
ù All client-server communication shall occur over HTTPS/TLS 1.2 or higher.
ù Real-time Household updates (FR-DASH-006, FR-NOTIF-002) shall use a persistent WebSocket connection with automatic reconnection and graceful fallback to polling if a WebSocket connection cannot be established.
...
High
NFR-REL-002
Manual transaction entry (FR-TXN-001) shall remain fully functional if the AI categorization service (Google Gemini) is unavailable, degrading only the category-suggestion step to a plain manual category picker.
High
NFR-REL-003
...
6.2 Data Retention
ù Transaction, Debt, and Budget data shall be retained for the lifetime of the Household unless deletion is explicitly requested.
ù Receipt images shall be retained for the lifetime of their linked Transaction(s) unless deletion is explicitly requested.
ù Upon a verified account-deletion request, personal financial data shall be deleted or irreversibly anonymized within the SLA defined in Section 7.1, consistent with NFR-SEC-005.
6.3 Data Migration
...
7.2 Localization
ù The initial release shall support English as the primary UI language, with the underlying architecture (externalized strings) supporting future localization without a rewrite.
ù The initial release shall support a single configurable base currency per Household; multi-currency support is Phase 3 scope (see Section 2.6).
7.3 Licensing
Third-party services integrated per Section 4.3 (Auth0, Stripe, Google Gemini, optional AWS Textract, and, in Phase 3, Plaid or equivalent) shall be used under their respective commercial terms of service; no requirement in this document presumes open-source licensing obligations beyond those already applicable to the organization's existing NestJS/Next.js stack.
